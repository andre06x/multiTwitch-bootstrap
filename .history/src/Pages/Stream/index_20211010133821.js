...
function Stream() {
  ...
  const streams = [];
  // streams.push({ channel: stream1 }, { channel: stream2 }, stream3 && { channel: stream3 }, stream4 && {channel: stream4});

  useEffect(() => {
    // setChat({ stream: [{ channel: stream1 }, { channel: stream2 }, stream3 && { channel: stream3 }, stream4 && {channel: stream4}], chat: stream1 })
